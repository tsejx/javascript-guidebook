...
在JavaScript中，变量的声明可以放在它的使用之后。换句话说，**变量可以先使用后声明**。这主要是因为JavaScript的提升(hoisting)机制在作怪。简单点说，**提升(hoisting)是JavaScript中默认就具有的一种机制，它将当用作用域内的所有声明都提升到所在作用域的最顶部**。如此一来，可以把变量提升归纳为：
