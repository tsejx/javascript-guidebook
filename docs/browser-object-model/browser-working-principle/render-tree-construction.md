...
在 [构建对象模型](constructing-the-object-model.md) 中，我们根据 HTML 和 CSS 输入构建了 DOM 树和 CSSOM 树。 不过，它们都是独立的对象，分别网罗文档不同方面的信息：一个描述内容，另一个则是描述需要对文档应用的样式规则。

DOM 树和 CSSOM 树将合并后在浏览器屏幕上渲染像素

* DOM 树与 CSSOM 树合并后形成渲染树
* 渲染树只包含渲染网页所需的节点
* [布局](layout.md) 计算每个对象的精确位置和大小
* 最后一步是 [绘制](painting.md)，使用最终渲染树将像素渲染到屏幕上
...
export default () => <img alt="渲染树构建" src={img} width={720} />;
```
...
export default () => <img alt="渲染树及对应DOM树" src={img} width={720} />;
```
